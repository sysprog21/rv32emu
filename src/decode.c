/*
 * rv32emu is freely redistributable under the MIT License. See the file
 * "LICENSE" for information on usage and redistribution of this file.
 */

/* AUTO-GENERATED by gen-decoder.py from instructions.in — DO NOT EDIT */

#include <assert.h>
#include <stdlib.h>

#include "decode.h"
#include "riscv_private.h"

/* decode rd field: insn[11:7] */
static inline uint32_t decode_rd(const uint32_t insn)
{
    return (insn >> 7) & 0x1f;
}

/* decode rs1 field: insn[19:15] */
static inline uint32_t decode_rs1(const uint32_t insn)
{
    return (insn >> 15) & 0x1f;
}

/* decode rs2 field: insn[24:20] */
static inline uint32_t decode_rs2(const uint32_t insn)
{
    return (insn >> 20) & 0x1f;
}

/* decode funct3 field: insn[14:12] — only used by F-extension (rounding mode)
 */
#if RV32_HAS(EXT_F)
static inline uint32_t decode_funct3(const uint32_t insn)
{
    return (insn >> 12) & 0x7;
}
#endif /* RV32_HAS(EXT_F) */


/* I-type immediate: insn[31:20] sign-extended */
static inline int32_t decode_itype_imm(const uint32_t insn)
{
    return ((int32_t) (insn & 0xFFF00000)) >> 20;
}

/* S-type immediate: insn[31:25|11:7] sign-extended */
static inline int32_t decode_stype_imm(const uint32_t insn)
{
    uint32_t dst = 0;
    dst |= (insn & 0xFE000000);
    dst |= (insn & 0x00000F80) << 13;
    return ((int32_t) dst) >> 20;
}

/* B-type immediate: insn[31|7|30:25|11:8] sign-extended */
static inline int32_t decode_btype_imm(const uint32_t insn)
{
    uint32_t dst = 0;
    dst |= (insn & 0x80000000);
    dst |= (insn & 0x00000080) << 23;
    dst |= (insn & 0x7E000000) >> 1;
    dst |= (insn & 0x00000F00) << 12;
    return ((int32_t) dst) >> 19;
}

/* U-type immediate: insn[31:12] */
static inline uint32_t decode_utype_imm(const uint32_t insn)
{
    return insn & 0xFFFFF000;
}

/* J-type immediate: insn[31|19:12|20|30:21] sign-extended */
static inline int32_t decode_jtype_imm(const uint32_t insn)
{
    uint32_t dst = 0;
    dst |= (insn & 0x80000000);
    dst |= (insn & 0x000FF000) << 11;
    dst |= (insn & 0x00100000) << 2;
    dst |= (insn & 0x7FE00000) >> 9;
    return ((int32_t) dst) >> 11;
}

#if RV32_HAS(EXT_F)
/* R4-type rs3 field: insn[31:27] */
static inline uint32_t decode_r4type_rs3(const uint32_t insn)
{
    return (insn >> 27) & 0x1f;
}
#endif

/* decode I-type */
static inline void decode_itype(rv_insn_t *ir, const uint32_t insn)
{
    ir->imm = decode_itype_imm(insn);
    ir->rs1 = decode_rs1(insn);
    ir->rd = decode_rd(insn);
}

/* decode U-type */
static inline void decode_utype(rv_insn_t *ir, const uint32_t insn)
{
    ir->imm = decode_utype_imm(insn);
    ir->rd = decode_rd(insn);
}

/* decode S-type */
static inline void decode_stype(rv_insn_t *ir, const uint32_t insn)
{
    ir->imm = decode_stype_imm(insn);
    ir->rs2 = decode_rs2(insn);
    ir->rs1 = decode_rs1(insn);
}

/* decode R-type */
static inline void decode_rtype(rv_insn_t *ir, const uint32_t insn)
{
    ir->rs2 = decode_rs2(insn);
    ir->rs1 = decode_rs1(insn);
    ir->rd = decode_rd(insn);
}

/* decode B-type */
static inline void decode_btype(rv_insn_t *ir, const uint32_t insn)
{
    ir->imm = decode_btype_imm(insn);
    ir->rs2 = decode_rs2(insn);
    ir->rs1 = decode_rs1(insn);
}

/* decode J-type */
static inline void decode_jtype(rv_insn_t *ir, const uint32_t insn)
{
    ir->imm = decode_jtype_imm(insn);
    ir->rd = decode_rd(insn);
}

#if RV32_HAS(EXT_F)
/* decode R4-type */
static inline void decode_r4type(rv_insn_t *ir, const uint32_t insn)
{
    ir->rd = decode_rd(insn);
    ir->rs1 = decode_rs1(insn);
    ir->rs2 = decode_rs2(insn);
    ir->rs3 = decode_r4type_rs3(insn);
    ir->rm = decode_funct3(insn);
}
#endif

/* Zicsr is the only extension with a post-decode check (see
 * _EXT_POST_CHECKS), and its leaves are the only callers, so the helper
 * has no users once Zicsr is disabled.  clang rejects an unused static
 * inline function under -Werror, so guard it the same way.
 */
#if RV32_HAS(Zicsr)
FORCE_INLINE bool csr_is_writable(const uint32_t csr)
{
    return csr < 0xc00;
}
#endif

#if RV32_HAS(EXT_V)
/* decode_veew: extract Effective Element Width from the width field
 * insn[14:12]. RVV EEW encoding: 000→8-bit, 101→16-bit, 110→32-bit, 111→64-bit.
 * Returns 8, 16, 32, or 64 (as uint8_t stored in ir->eew).
 * Matches the decode_eew() + (8 << eew) calculation in decode_v.c.
 */
static inline uint8_t decode_veew(const uint32_t insn)
{
    const uint8_t w = (uint8_t) ((insn >> 12) & 0x7u);
    /* w∈{0,1,2,3} → eew_exp=w; w∈{5,6,7} → eew_exp=w-4 (1,2,3) */
    const uint8_t e = (w < 4u) ? w : (uint8_t) (w - 4u);
    return (uint8_t) (8u << e);
}
#endif /* RV32_HAS(EXT_V) */

static inline bool op_load(rv_insn_t *ir, const uint32_t insn)
{
    decode_itype(ir, insn);

    switch ((insn >> 12) & 0x7) {
    case 0:
        ir->opcode = rv_insn_lb;
        return true;
    case 1:
        ir->opcode = rv_insn_lh;
        return true;
    case 2:
        ir->opcode = rv_insn_lw;
        return true;
    case 4:
        ir->opcode = rv_insn_lbu;
        return true;
    case 5:
        ir->opcode = rv_insn_lhu;
        return true;
    default:
        break;
    }
    return false;
}

#if RV32_HAS(EXT_F) || RV32_HAS(EXT_V)
static inline bool op_load_fp(rv_insn_t *ir, const uint32_t insn)
{
    decode_itype(ir, insn);

    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_F)
    case 2:
        ir->opcode = rv_insn_flw;
        return true;
#endif /* RV32_HAS(EXT_F) */
    default:
        break;
    }
    switch ((insn >> 25) & 0x1) {
    case 1:
        switch ((insn >> 29) & 0x7) {
        case 0:
            switch ((insn >> 20) & 0x1f) {
            case 0x8:
                switch ((insn >> 28) & 0x1) {
                case 0:
                    switch ((insn >> 26) & 0x3) {
                    case 0:
                        switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl1re8_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 5:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl1re16_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 6:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl1re32_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 7:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl1re64_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            case 0xb:
                switch ((insn >> 28) & 0x1) {
                case 0:
                    switch ((insn >> 26) & 0x3) {
                    case 0:
                        switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vlm_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 1:
            switch ((insn >> 20) & 0x1f) {
            case 0x8:
                switch ((insn >> 28) & 0x1) {
                case 0:
                    switch ((insn >> 26) & 0x3) {
                    case 0:
                        switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl2re8_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 5:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl2re16_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 6:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl2re32_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 7:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl2re64_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 3:
            switch ((insn >> 20) & 0x1f) {
            case 0x8:
                switch ((insn >> 28) & 0x1) {
                case 0:
                    switch ((insn >> 26) & 0x3) {
                    case 0:
                        switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl4re8_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 5:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl4re16_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 6:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl4re32_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 7:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl4re64_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 7:
            switch ((insn >> 20) & 0x1f) {
            case 0x8:
                switch ((insn >> 28) & 0x1) {
                case 0:
                    switch ((insn >> 26) & 0x3) {
                    case 0:
                        switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl8re8_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 5:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl8re16_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 6:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl8re32_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                        case 7:
                            ir->vd = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vl8re64_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 29) & 0x7) {
    case 0:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vle8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vle16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vle32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vle64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x10:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vle8ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vle16ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vle32ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vle64ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlse8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlse16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlse32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlse64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 1:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg2e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg2e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg2e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg2e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x10:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg2e8ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg2e16ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg2e32ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg2e64ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg2ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg2ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg2ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg2ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg2e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg2e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg2e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg2e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg2ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg2ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg2ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg2ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 2:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg3e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg3e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg3e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg3e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x10:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg3e8ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg3e16ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg3e32ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg3e64ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg3ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg3ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg3ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg3ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg3e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg3e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg3e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg3e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg3ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg3ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg3ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg3ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 3:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg4e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg4e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg4e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg4e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x10:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg4e8ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg4e16ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg4e32ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg4e64ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg4ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg4ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg4ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg4ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg4e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg4e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg4e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg4e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg4ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg4ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg4ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg4ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 4:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg5e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg5e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg5e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg5e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x10:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg5e8ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg5e16ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg5e32ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg5e64ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg5ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg5ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg5ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg5ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg5e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg5e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg5e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg5e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg5ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg5ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg5ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg5ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 5:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg6e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg6e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg6e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg6e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x10:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg6e8ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg6e16ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg6e32ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg6e64ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg6ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg6ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg6ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg6ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg6e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg6e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg6e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg6e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg6ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg6ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg6ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg6ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 6:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg7e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg7e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg7e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg7e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x10:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg7e8ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg7e16ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg7e32ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg7e64ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg7ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg7ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg7ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg7ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg7e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg7e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg7e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg7e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg7ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg7ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg7ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg7ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 7:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg8e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg8e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg8e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg8e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x10:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg8e8ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg8e16ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg8e32ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vd = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vlseg8e64ff_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg8ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg8ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg8ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vluxseg8ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg8e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg8e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg8e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vlsseg8e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg8ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg8ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg8ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vd = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vloxseg8ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    return false;
}
#else /* !RV32_HAS(EXT_F) && !RV32_HAS(EXT_V) */
#define op_load_fp op_unimp
#endif /* RV32_HAS(EXT_F) || RV32_HAS(EXT_V) */

static inline bool op_misc_mem(rv_insn_t *ir, const uint32_t insn)
{
    switch ((insn >> 12) & 0x7) {
    case 0:
        ir->opcode = rv_insn_fence;
        return true;
#if RV32_HAS(Zifencei)
    case 1:
        ir->opcode = rv_insn_fencei;
        return true;
#endif /* RV32_HAS(Zifencei) */
    default:
        break;
    }
    return false;
}

static inline bool op_op_imm(rv_insn_t *ir, const uint32_t insn)
{
    decode_itype(ir, insn);

    switch ((insn >> 12) & 0x7) {
    case 0:
        ir->opcode =
            unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_addi;
        return true;
    case 1:
        switch ((insn >> 20) & 0xfff) {
#if RV32_HAS(Zbb)
        case 0x600:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_clz;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zbb)
        case 0x601:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_ctz;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zbb)
        case 0x602:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_cpop;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zbb)
        case 0x604:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sextb;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zbb)
        case 0x605:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sexth;
            return true;
#endif /* RV32_HAS(Zbb) */
        default:
            break;
        }
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_slli;
            return true;
#if RV32_HAS(Zbs)
        case 0x14:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_bseti;
            return true;
#endif /* RV32_HAS(Zbs) */
#if RV32_HAS(Zbs)
        case 0x24:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_bclri;
            return true;
#endif /* RV32_HAS(Zbs) */
#if RV32_HAS(Zbs)
        case 0x34:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_binvi;
            return true;
#endif /* RV32_HAS(Zbs) */
        default:
            break;
        }
        break;
    case 2:
        ir->opcode =
            unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_slti;
        return true;
    case 3:
        ir->opcode =
            unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sltiu;
        return true;
    case 4:
        ir->opcode =
            unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_xori;
        return true;
    case 5:
        switch ((insn >> 20) & 0xfff) {
#if RV32_HAS(Zbb)
        case 0x287:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_orcb;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zbb)
        case 0x698:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_rev8;
            return true;
#endif /* RV32_HAS(Zbb) */
        default:
            break;
        }
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_srli;
            return true;
        case 0x20:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_srai;
            return true;
#if RV32_HAS(Zbs)
        case 0x24:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_bexti;
            return true;
#endif /* RV32_HAS(Zbs) */
#if RV32_HAS(Zbb)
        case 0x30:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_rori;
            return true;
#endif /* RV32_HAS(Zbb) */
        default:
            break;
        }
        break;
    case 6:
        ir->opcode =
            unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_ori;
        return true;
    case 7:
        ir->opcode =
            unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_andi;
        return true;
    default:
        break;
    }
    return false;
}

static inline bool op_auipc(rv_insn_t *ir, const uint32_t insn)
{
    decode_utype(ir, insn);
    ir->opcode = unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_auipc;
    return true;
}

static inline bool op_store(rv_insn_t *ir, const uint32_t insn)
{
    decode_stype(ir, insn);

    switch ((insn >> 12) & 0x7) {
    case 0:
        ir->opcode = rv_insn_sb;
        return true;
    case 1:
        ir->opcode = rv_insn_sh;
        return true;
    case 2:
        ir->opcode = rv_insn_sw;
        return true;
    default:
        break;
    }
    return false;
}

#if RV32_HAS(EXT_F) || RV32_HAS(EXT_V)
static inline bool op_store_fp(rv_insn_t *ir, const uint32_t insn)
{
    decode_stype(ir, insn);

    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_F)
    case 2:
        ir->opcode = rv_insn_fsw;
        return true;
#endif /* RV32_HAS(EXT_F) */
    default:
        break;
    }
    switch ((insn >> 25) & 0x1) {
    case 1:
        switch ((insn >> 12) & 0x7) {
        case 0:
            switch ((insn >> 29) & 0x7) {
            case 0:
                switch ((insn >> 20) & 0x1f) {
                case 0x8:
                    switch ((insn >> 28) & 0x1) {
                    case 0:
                        switch ((insn >> 26) & 0x3) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vs3 = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vs1r_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            case 1:
                switch ((insn >> 20) & 0x1f) {
                case 0x8:
                    switch ((insn >> 28) & 0x1) {
                    case 0:
                        switch ((insn >> 26) & 0x3) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vs3 = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vs2r_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 20) & 0x1f) {
                case 0x8:
                    switch ((insn >> 28) & 0x1) {
                    case 0:
                        switch ((insn >> 26) & 0x3) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vs3 = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vs4r_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            case 7:
                switch ((insn >> 20) & 0x1f) {
                case 0x8:
                    switch ((insn >> 28) & 0x1) {
                    case 0:
                        switch ((insn >> 26) & 0x3) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vs3 = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vs8r_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 29) & 0x7) {
        case 0:
            switch ((insn >> 20) & 0x1f) {
            case 0xb:
                switch ((insn >> 28) & 0x1) {
                case 0:
                    switch ((insn >> 26) & 0x3) {
                    case 0:
                        switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                        case 0:
                            ir->vs3 = decode_rd(insn);
                            ir->rs1 = decode_rs1(insn);
                            ir->vm = (insn >> 25) & 0x1;
                            ir->eew = decode_veew(insn);
                            ir->opcode = rv_insn_vsm_v;
                            return true;
#endif /* RV32_HAS(EXT_V) */
                        default:
                            break;
                        }
                        break;
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 29) & 0x7) {
    case 0:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vse8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vse16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vse32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vse64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsse8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsse16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsse32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsse64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 1:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg2e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg2e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg2e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg2e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg2ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg2ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg2ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg2ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg2e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg2e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg2e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg2e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg2ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg2ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg2ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg2ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 2:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg3e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg3e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg3e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg3e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg3ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg3ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg3ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg3ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg3e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg3e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg3e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg3e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg3ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg3ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg3ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg3ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 3:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg4e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg4e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg4e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg4e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg4ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg4ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg4ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg4ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg4e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg4e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg4e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg4e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg4ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg4ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg4ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg4ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 4:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg5e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg5e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg5e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg5e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg5ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg5ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg5ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg5ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg5e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg5e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg5e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg5e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg5ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg5ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg5ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg5ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 5:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg6e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg6e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg6e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg6e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg6ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg6ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg6ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg6ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg6e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg6e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg6e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg6e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg6ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg6ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg6ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg6ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 6:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg7e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg7e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg7e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg7e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg7ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg7ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg7ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg7ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg7e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg7e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg7e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg7e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg7ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg7ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg7ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg7ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 7:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 28) & 0x1) {
            case 0:
                switch ((insn >> 26) & 0x3) {
                case 0:
                    switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                    case 0:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg8e8_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 5:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg8e16_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 6:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg8e32_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                    case 7:
                        ir->vs3 = decode_rd(insn);
                        ir->rs1 = decode_rs1(insn);
                        ir->vm = (insn >> 25) & 0x1;
                        ir->eew = decode_veew(insn);
                        ir->opcode = rv_insn_vsseg8e64_v;
                        return true;
#endif /* RV32_HAS(EXT_V) */
                    default:
                        break;
                    }
                    break;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 28) & 0x1) {
        case 0:
            switch ((insn >> 26) & 0x3) {
            case 1:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg8ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg8ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg8ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsuxseg8ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 2:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg8e8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg8e16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg8e32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->rs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vssseg8e64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            case 3:
                switch ((insn >> 12) & 0x7) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg8ei8_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 5:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg8ei16_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 6:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg8ei32_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
                case 7:
                    ir->vs3 = decode_rd(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->eew = decode_veew(insn);
                    ir->opcode = rv_insn_vsoxseg8ei64_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    return false;
}
#else /* !RV32_HAS(EXT_F) && !RV32_HAS(EXT_V) */
#define op_store_fp op_unimp
#endif /* RV32_HAS(EXT_F) || RV32_HAS(EXT_V) */

static inline bool op_amo(rv_insn_t *ir, const uint32_t insn)
{
    decode_rtype(ir, insn);

    switch ((insn >> 12) & 0x7) {
    case 2:
        switch ((insn >> 27) & 0x1f) {
#if RV32_HAS(EXT_A)
        case 0:
            ir->opcode = rv_insn_amoaddw;
            return true;
#endif /* RV32_HAS(EXT_A) */
#if RV32_HAS(EXT_A)
        case 1:
            ir->opcode = rv_insn_amoswapw;
            return true;
#endif /* RV32_HAS(EXT_A) */
        case 2:
            switch ((insn >> 20) & 0x1f) {
#if RV32_HAS(EXT_A)
            case 0:
                ir->opcode = rv_insn_lrw;
                return true;
#endif /* RV32_HAS(EXT_A) */
            default:
                break;
            }
            break;
#if RV32_HAS(EXT_A)
        case 3:
            ir->opcode = rv_insn_scw;
            return true;
#endif /* RV32_HAS(EXT_A) */
#if RV32_HAS(EXT_A)
        case 4:
            ir->opcode = rv_insn_amoxorw;
            return true;
#endif /* RV32_HAS(EXT_A) */
#if RV32_HAS(EXT_A)
        case 0x8:
            ir->opcode = rv_insn_amoorw;
            return true;
#endif /* RV32_HAS(EXT_A) */
#if RV32_HAS(EXT_A)
        case 0xc:
            ir->opcode = rv_insn_amoandw;
            return true;
#endif /* RV32_HAS(EXT_A) */
#if RV32_HAS(EXT_A)
        case 0x10:
            ir->opcode = rv_insn_amominw;
            return true;
#endif /* RV32_HAS(EXT_A) */
#if RV32_HAS(EXT_A)
        case 0x14:
            ir->opcode = rv_insn_amomaxw;
            return true;
#endif /* RV32_HAS(EXT_A) */
#if RV32_HAS(EXT_A)
        case 0x18:
            ir->opcode = rv_insn_amominuw;
            return true;
#endif /* RV32_HAS(EXT_A) */
#if RV32_HAS(EXT_A)
        case 0x1c:
            ir->opcode = rv_insn_amomaxuw;
            return true;
#endif /* RV32_HAS(EXT_A) */
        default:
            break;
        }
        break;
    default:
        break;
    }
    return false;
}

static inline bool op_op(rv_insn_t *ir, const uint32_t insn)
{
    decode_rtype(ir, insn);

    switch ((insn >> 12) & 0x7) {
    case 0:
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_add;
            return true;
#if RV32_HAS(EXT_M)
        case 1:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_mul;
            return true;
#endif /* RV32_HAS(EXT_M) */
        case 0x20:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sub;
            return true;
        default:
            break;
        }
        break;
    case 1:
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sll;
            return true;
#if RV32_HAS(EXT_M)
        case 1:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_mulh;
            return true;
#endif /* RV32_HAS(EXT_M) */
#if RV32_HAS(Zbc)
        case 5:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_clmul;
            return true;
#endif /* RV32_HAS(Zbc) */
#if RV32_HAS(Zbs)
        case 0x14:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_bset;
            return true;
#endif /* RV32_HAS(Zbs) */
#if RV32_HAS(Zbs)
        case 0x24:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_bclr;
            return true;
#endif /* RV32_HAS(Zbs) */
#if RV32_HAS(Zbb)
        case 0x30:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_rol;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zbs)
        case 0x34:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_binv;
            return true;
#endif /* RV32_HAS(Zbs) */
        default:
            break;
        }
        break;
    case 2:
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_slt;
            return true;
#if RV32_HAS(EXT_M)
        case 1:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_mulhsu;
            return true;
#endif /* RV32_HAS(EXT_M) */
#if RV32_HAS(Zbc)
        case 5:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_clmulr;
            return true;
#endif /* RV32_HAS(Zbc) */
#if RV32_HAS(Zba)
        case 0x10:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sh1add;
            return true;
#endif /* RV32_HAS(Zba) */
        default:
            break;
        }
        break;
    case 3:
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sltu;
            return true;
#if RV32_HAS(EXT_M)
        case 1:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_mulhu;
            return true;
#endif /* RV32_HAS(EXT_M) */
#if RV32_HAS(Zbc)
        case 5:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_clmulh;
            return true;
#endif /* RV32_HAS(Zbc) */
        default:
            break;
        }
        break;
    case 4:
        switch ((insn >> 20) & 0x1f) {
        case 0:
            switch ((insn >> 25) & 0x7f) {
#if RV32_HAS(Zbb)
            case 4:
                ir->opcode = unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop
                                                             : rv_insn_zexth;
                return true;
#endif /* RV32_HAS(Zbb) */
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_xor;
            return true;
#if RV32_HAS(EXT_M)
        case 1:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_div;
            return true;
#endif /* RV32_HAS(EXT_M) */
#if RV32_HAS(Zbb)
        case 5:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_min;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zba)
        case 0x10:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sh2add;
            return true;
#endif /* RV32_HAS(Zba) */
#if RV32_HAS(Zbb)
        case 0x20:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_xnor;
            return true;
#endif /* RV32_HAS(Zbb) */
        default:
            break;
        }
        break;
    case 5:
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_srl;
            return true;
#if RV32_HAS(EXT_M)
        case 1:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_divu;
            return true;
#endif /* RV32_HAS(EXT_M) */
#if RV32_HAS(Zbb)
        case 5:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_minu;
            return true;
#endif /* RV32_HAS(Zbb) */
        case 0x20:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sra;
            return true;
#if RV32_HAS(Zbs)
        case 0x24:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_bext;
            return true;
#endif /* RV32_HAS(Zbs) */
#if RV32_HAS(Zbb)
        case 0x30:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_ror;
            return true;
#endif /* RV32_HAS(Zbb) */
        default:
            break;
        }
        break;
    case 6:
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_or;
            return true;
#if RV32_HAS(EXT_M)
        case 1:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_rem;
            return true;
#endif /* RV32_HAS(EXT_M) */
#if RV32_HAS(Zbb)
        case 5:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_max;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zba)
        case 0x10:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_sh3add;
            return true;
#endif /* RV32_HAS(Zba) */
#if RV32_HAS(Zbb)
        case 0x20:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_orn;
            return true;
#endif /* RV32_HAS(Zbb) */
        default:
            break;
        }
        break;
    case 7:
        switch ((insn >> 25) & 0x7f) {
        case 0:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_and;
            return true;
#if RV32_HAS(EXT_M)
        case 1:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_remu;
            return true;
#endif /* RV32_HAS(EXT_M) */
#if RV32_HAS(Zbb)
        case 5:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_maxu;
            return true;
#endif /* RV32_HAS(Zbb) */
#if RV32_HAS(Zbb)
        case 0x20:
            ir->opcode =
                unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_andn;
            return true;
#endif /* RV32_HAS(Zbb) */
        default:
            break;
        }
        break;
    default:
        break;
    }
    return false;
}

static inline bool op_lui(rv_insn_t *ir, const uint32_t insn)
{
    decode_utype(ir, insn);
    ir->opcode = unlikely(ir->rd == rv_reg_zero) ? rv_insn_nop : rv_insn_lui;
    return true;
}

#if RV32_HAS(EXT_F)
static inline bool op_madd(rv_insn_t *ir, const uint32_t insn)
{
    decode_r4type(ir, insn);
    ir->rm = decode_funct3(insn); /* rounding mode */

    switch ((insn >> 25) & 0x3) {
#if RV32_HAS(EXT_F)
    case 0:
        ir->opcode = rv_insn_fmadds;
        return true;
#endif /* RV32_HAS(EXT_F) */
    default:
        break;
    }
    return false;
}
#else /* !RV32_HAS(EXT_F) */
#define op_madd op_unimp
#endif /* RV32_HAS(EXT_F) */

#if RV32_HAS(EXT_F)
static inline bool op_msub(rv_insn_t *ir, const uint32_t insn)
{
    decode_r4type(ir, insn);
    ir->rm = decode_funct3(insn); /* rounding mode */

    switch ((insn >> 25) & 0x3) {
#if RV32_HAS(EXT_F)
    case 0:
        ir->opcode = rv_insn_fmsubs;
        return true;
#endif /* RV32_HAS(EXT_F) */
    default:
        break;
    }
    return false;
}
#else /* !RV32_HAS(EXT_F) */
#define op_msub op_unimp
#endif /* RV32_HAS(EXT_F) */

#if RV32_HAS(EXT_F)
static inline bool op_nmsub(rv_insn_t *ir, const uint32_t insn)
{
    decode_r4type(ir, insn);
    ir->rm = decode_funct3(insn); /* rounding mode */

    switch ((insn >> 25) & 0x3) {
#if RV32_HAS(EXT_F)
    case 0:
        ir->opcode = rv_insn_fnmsubs;
        return true;
#endif /* RV32_HAS(EXT_F) */
    default:
        break;
    }
    return false;
}
#else /* !RV32_HAS(EXT_F) */
#define op_nmsub op_unimp
#endif /* RV32_HAS(EXT_F) */

#if RV32_HAS(EXT_F)
static inline bool op_nmadd(rv_insn_t *ir, const uint32_t insn)
{
    decode_r4type(ir, insn);
    ir->rm = decode_funct3(insn); /* rounding mode */

    switch ((insn >> 25) & 0x3) {
#if RV32_HAS(EXT_F)
    case 0:
        ir->opcode = rv_insn_fnmadds;
        return true;
#endif /* RV32_HAS(EXT_F) */
    default:
        break;
    }
    return false;
}
#else /* !RV32_HAS(EXT_F) */
#define op_nmadd op_unimp
#endif /* RV32_HAS(EXT_F) */

#if RV32_HAS(EXT_F)
static inline bool op_op_fp(rv_insn_t *ir, const uint32_t insn)
{
    decode_rtype(ir, insn);
    ir->rm = decode_funct3(insn); /* rounding mode */

    switch ((insn >> 25) & 0x3) {
    case 0:
        switch ((insn >> 12) & 0x7) {
        case 0:
            switch ((insn >> 27) & 0x1f) {
#if RV32_HAS(EXT_F)
            case 4:
                ir->opcode = rv_insn_fsgnjs;
                return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
            case 5:
                ir->opcode = rv_insn_fmins;
                return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
            case 0x14:
                ir->opcode = rv_insn_fles;
                return true;
#endif /* RV32_HAS(EXT_F) */
            case 0x1c:
                switch ((insn >> 20) & 0x1f) {
#if RV32_HAS(EXT_F)
                case 0:
                    ir->opcode = rv_insn_fmvxw;
                    return true;
#endif /* RV32_HAS(EXT_F) */
                default:
                    break;
                }
                break;
            case 0x1e:
                switch ((insn >> 20) & 0x1f) {
#if RV32_HAS(EXT_F)
                case 0:
                    ir->opcode = rv_insn_fmvwx;
                    return true;
#endif /* RV32_HAS(EXT_F) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 1:
            switch ((insn >> 27) & 0x1f) {
#if RV32_HAS(EXT_F)
            case 4:
                ir->opcode = rv_insn_fsgnjns;
                return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
            case 5:
                ir->opcode = rv_insn_fmaxs;
                return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
            case 0x14:
                ir->opcode = rv_insn_flts;
                return true;
#endif /* RV32_HAS(EXT_F) */
            case 0x1c:
                switch ((insn >> 20) & 0x1f) {
#if RV32_HAS(EXT_F)
                case 0:
                    ir->opcode = rv_insn_fclasss;
                    return true;
#endif /* RV32_HAS(EXT_F) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 2:
            switch ((insn >> 27) & 0x1f) {
#if RV32_HAS(EXT_F)
            case 4:
                ir->opcode = rv_insn_fsgnjxs;
                return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
            case 0x14:
                ir->opcode = rv_insn_feqs;
                return true;
#endif /* RV32_HAS(EXT_F) */
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 27) & 0x1f) {
#if RV32_HAS(EXT_F)
        case 0:
            ir->opcode = rv_insn_fadds;
            return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
        case 1:
            ir->opcode = rv_insn_fsubs;
            return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
        case 2:
            ir->opcode = rv_insn_fmuls;
            return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
        case 3:
            ir->opcode = rv_insn_fdivs;
            return true;
#endif /* RV32_HAS(EXT_F) */
        case 0xb:
            switch ((insn >> 20) & 0x1f) {
#if RV32_HAS(EXT_F)
            case 0:
                ir->opcode = rv_insn_fsqrts;
                return true;
#endif /* RV32_HAS(EXT_F) */
            default:
                break;
            }
            break;
        case 0x18:
            switch ((insn >> 20) & 0x1f) {
#if RV32_HAS(EXT_F)
            case 0:
                ir->opcode = rv_insn_fcvtws;
                return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
            case 1:
                ir->opcode = rv_insn_fcvtwus;
                return true;
#endif /* RV32_HAS(EXT_F) */
            default:
                break;
            }
            break;
        case 0x1a:
            switch ((insn >> 20) & 0x1f) {
#if RV32_HAS(EXT_F)
            case 0:
                ir->opcode = rv_insn_fcvtsw;
                return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_F)
            case 1:
                ir->opcode = rv_insn_fcvtswu;
                return true;
#endif /* RV32_HAS(EXT_F) */
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    return false;
}
#else /* !RV32_HAS(EXT_F) */
#define op_op_fp op_unimp
#endif /* RV32_HAS(EXT_F) */

#if RV32_HAS(EXT_V)
static inline bool op_op_v(rv_insn_t *ir, const uint32_t insn)
{
    switch ((insn >> 12) & 0x7) {
    case 0:
        switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
        case 0:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 2:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 4:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vminu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 5:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmin_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 6:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmaxu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 7:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmax_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x9:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vand_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xa:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vor_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xb:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vxor_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xc:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vrgather_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xe:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vrgatherei16_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
        case 0x17:
            switch ((insn >> 25) & 0x1) {
#if RV32_HAS(EXT_V)
            case 0:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmerge_vvm;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
#if RV32_HAS(EXT_V)
        case 0x18:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmseq_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x19:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsne_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsltu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmslt_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsleu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsle_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x20:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsaddu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x21:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x22:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssubu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x23:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x25:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsll_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x27:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsmul_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x28:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsrl_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x29:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsra_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssrl_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssra_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnsrl_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnsra_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnclipu_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnclip_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x30:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwredsumu_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x31:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwredsum_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        break;
    case 1:
        switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
        case 0:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 1:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfredusum_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 2:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfsub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 3:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfredosum_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 4:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmin_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 5:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfredmin_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 6:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmax_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 7:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfredmax_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x8:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfsgnj_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x9:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfsgnjn_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xa:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfsgnjx_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x18:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmfeq_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x19:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmfle_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmflt_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmfne_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x20:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfdiv_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x24:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmul_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x28:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x29:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfnmadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmsub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfnmsub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmacc_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfnmacc_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmsac_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfnmsac_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x30:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x31:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwredusum_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x32:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwsub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x33:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwredosum_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x34:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwadd_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x36:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwsub_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x38:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwmul_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwmacc_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwnmacc_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwmsac_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwnmsac_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        break;
    case 2:
        switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
        case 0:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vredsum_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 1:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vredand_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 2:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vredor_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 3:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vredxor_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 4:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vredminu_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 5:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vredmin_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 6:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vredmaxu_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 7:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vredmax_vs;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x8:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vaaddu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x9:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vaadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xa:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vasubu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xb:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vasub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
        case 0x10:
            switch ((insn >> 15) & 0x1f) {
#if RV32_HAS(EXT_V)
            case 0x10:
                ir->rd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vcpop_m;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x11:
                ir->rd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vfirst_m;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
        case 0x14:
            switch ((insn >> 15) & 0x1f) {
#if RV32_HAS(EXT_V)
            case 1:
                ir->rd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmsbf_m;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 2:
                ir->rd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmsof_m;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 3:
                ir->rd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmsif_m;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x10:
                ir->rd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_viota_m;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
#if RV32_HAS(EXT_V)
        case 0x20:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vdivu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x21:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vdiv_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x22:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vremu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x23:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vrem_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x24:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmulhu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x25:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmul_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x26:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmulhsu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x27:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmulh_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x29:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnmsub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmacc_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnmsac_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x30:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwaddu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x31:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwadd_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x32:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwsubu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x33:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwsub_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x34:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwaddu_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x35:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwadd_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x36:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwsubu_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x37:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwsub_wv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x38:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmulu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmulsu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmul_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmaccu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmacc_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->vs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmaccsu_vv;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        break;
    case 3:
        switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
        case 0:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vadd_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 3:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vrsub_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x9:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vand_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xa:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vor_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xb:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vxor_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xc:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vrgather_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xe:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vslideup_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xf:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vslidedown_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
        case 0x17:
            switch ((insn >> 25) & 0x1) {
#if RV32_HAS(EXT_V)
            case 0:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->imm = ((int32_t) (insn << 12)) >> 27;
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmerge_vim;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
#if RV32_HAS(EXT_V)
        case 0x18:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmseq_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x19:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsne_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsleu_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsle_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsgtu_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsgt_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x20:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsaddu_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x21:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsadd_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x25:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsll_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x28:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsrl_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x29:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsra_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssrl_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssra_vi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnsrl_wi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnsra_wi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnclipu_wi;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->imm = ((int32_t) (insn << 12)) >> 27;
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnclip_wi;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        break;
    case 4:
        switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
        case 0:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vadd_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 2:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsub_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 3:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vrsub_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 4:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vminu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 5:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmin_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 6:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmaxu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 7:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmax_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x9:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vand_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xa:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vor_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xb:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vxor_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xc:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vrgather_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xe:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vslideup_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xf:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vslidedown_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
        case 0x17:
            switch ((insn >> 25) & 0x1) {
#if RV32_HAS(EXT_V)
            case 0:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->rs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmerge_vxm;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
#if RV32_HAS(EXT_V)
        case 0x18:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmseq_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x19:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsne_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsltu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmslt_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsleu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsle_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsgtu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmsgt_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x20:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsaddu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x21:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsadd_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x22:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssubu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x23:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssub_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x25:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsll_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x27:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsmul_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x28:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsrl_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x29:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vsra_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssrl_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vssra_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnsrl_wx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnsra_wx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnclipu_wx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnclip_wx;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        break;
    case 5:
        switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
        case 0:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfadd_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 2:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfsub_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 4:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmin_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 6:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmax_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x8:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfsgnj_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x9:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfsgnjn_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xa:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfsgnjx_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xe:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfslide1up_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xf:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfslide1down_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
        case 0x17:
            switch ((insn >> 25) & 0x1) {
#if RV32_HAS(EXT_V)
            case 0:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->rs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vfmerge_vfm;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
#if RV32_HAS(EXT_V)
        case 0x18:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmfeq_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x19:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmfle_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmflt_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmfne_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmfgt_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x1f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmfge_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x20:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfdiv_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x21:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfrdiv_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x24:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmul_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x27:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfrsub_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x28:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmadd_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x29:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfnmadd_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmsub_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfnmsub_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmacc_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfnmacc_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfmsac_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfnmsac_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x30:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwadd_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x32:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwsub_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x34:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwadd_wf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x36:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwsub_wf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x38:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwmul_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwmacc_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwnmacc_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwmsac_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vfwnmsac_vf;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        break;
    case 6:
        switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
        case 0x8:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vaaddu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x9:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vaadd_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xa:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vasubu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xb:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vasub_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xe:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vslide1up_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0xf:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vslide1down_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x20:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vdivu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x21:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vdiv_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x22:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vremu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x23:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vrem_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x24:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmulhu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x25:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmul_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x26:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmulhsu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x27:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmulh_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x29:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmadd_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnmsub_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vmacc_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x2f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vnmsac_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x30:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwaddu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x31:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwadd_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x32:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwsubu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x33:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwsub_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x34:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwaddu_wx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x35:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwadd_wx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x36:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwsubu_wx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x37:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwsub_wx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x38:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmulu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3a:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmulsu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3b:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmul_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3c:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmaccu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3d:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmacc_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3e:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmaccus_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
        case 0x3f:
            ir->vd = decode_rd(insn);
            ir->vs2 = decode_rs2(insn);
            ir->rs1 = decode_rs1(insn);
            ir->vm = (insn >> 25) & 0x1;
            ir->opcode = rv_insn_vwmaccsu_vx;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        break;
    case 7:
        switch ((insn >> 25) & 0x7f) {
#if RV32_HAS(EXT_V)
        case 0x40:
            ir->rd = decode_rd(insn);
            ir->rs1 = decode_rs1(insn);
            ir->rs2 = decode_rs2(insn);
            ir->opcode = rv_insn_vsetvl;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        switch ((insn >> 30) & 0x3) {
#if RV32_HAS(EXT_V)
        case 3:
            ir->rd = decode_rd(insn);
            ir->rs1 = (insn >> 15) & 0x1f;
            ir->zimm = (insn >> 20) & 0x3ff;
            ir->opcode = rv_insn_vsetivli;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        switch ((insn >> 31) & 0x1) {
#if RV32_HAS(EXT_V)
        case 0:
            ir->rd = decode_rd(insn);
            ir->rs1 = decode_rs1(insn);
            ir->zimm = (insn >> 20) & 0x7ff;
            ir->opcode = rv_insn_vsetvli;
            return true;
#endif /* RV32_HAS(EXT_V) */
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 20) & 0x1f) {
    case 0:
        switch ((insn >> 12) & 0x7) {
        case 0:
            switch ((insn >> 26) & 0x3f) {
            case 0x17:
                switch ((insn >> 25) & 0x1) {
#if RV32_HAS(EXT_V)
                case 1:
                    ir->vd = decode_rd(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vs1 = decode_rs1(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->opcode = rv_insn_vmv_v_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 2:
            switch ((insn >> 26) & 0x3f) {
            case 0x14:
                switch ((insn >> 15) & 0x1f) {
#if RV32_HAS(EXT_V)
                case 0x11:
                    ir->vd = decode_rd(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->opcode = rv_insn_vid_v;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 3:
            switch ((insn >> 26) & 0x3f) {
            case 0x17:
                switch ((insn >> 25) & 0x1) {
#if RV32_HAS(EXT_V)
                case 1:
                    ir->vd = decode_rd(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->imm = ((int32_t) (insn << 12)) >> 27;
                    ir->vm = (insn >> 25) & 0x1;
                    ir->opcode = rv_insn_vmv_v_i;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 4:
            switch ((insn >> 26) & 0x3f) {
            case 0x17:
                switch ((insn >> 25) & 0x1) {
#if RV32_HAS(EXT_V)
                case 1:
                    ir->vd = decode_rd(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->opcode = rv_insn_vmv_v_x;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 5:
            switch ((insn >> 26) & 0x3f) {
            case 0x17:
                switch ((insn >> 25) & 0x1) {
#if RV32_HAS(EXT_V)
                case 1:
                    ir->vd = decode_rd(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->opcode = rv_insn_vfmv_v_f;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 25) & 0x1) {
        case 1:
            switch ((insn >> 12) & 0x7) {
            case 6:
                switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
                case 0x10:
                    ir->vd = decode_rd(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->rs1 = decode_rs1(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->opcode = rv_insn_vmv_s_x;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 25) & 0x1) {
    case 0:
        switch ((insn >> 12) & 0x7) {
        case 0:
            switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
            case 0x10:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vadc_vvm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x12:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vsbc_vvm;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
        case 3:
            switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
            case 0x10:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->imm = ((int32_t) (insn << 12)) >> 27;
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vadc_vim;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
        case 4:
            switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
            case 0x10:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->rs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vadc_vxm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x12:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->rs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vsbc_vxm;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 1:
        switch ((insn >> 12) & 0x7) {
        case 0:
            switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
            case 0x11:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmadc_vv;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x13:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmsbc_vv;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
        case 2:
            switch ((insn >> 26) & 0x3f) {
            case 0x10:
                switch ((insn >> 15) & 0x1f) {
#if RV32_HAS(EXT_V)
                case 0:
                    ir->rd = decode_rd(insn);
                    ir->vs2 = decode_rs2(insn);
                    ir->vm = (insn >> 25) & 0x1;
                    ir->opcode = rv_insn_vmv_x_s;
                    return true;
#endif /* RV32_HAS(EXT_V) */
                default:
                    break;
                }
                break;
#if RV32_HAS(EXT_V)
            case 0x17:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vcompress_vm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x18:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmandn_mm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x19:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmand_mm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x1a:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmor_mm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x1b:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmxor_mm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x1c:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmorn_mm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x1d:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmnand_mm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x1e:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmnor_mm;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x1f:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->vs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmxnor_mm;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
        case 3:
            switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
            case 0x11:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->imm = ((int32_t) (insn << 12)) >> 27;
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmadc_vi;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
        case 4:
            switch ((insn >> 26) & 0x3f) {
#if RV32_HAS(EXT_V)
            case 0x11:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->rs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmadc_vx;
                return true;
#endif /* RV32_HAS(EXT_V) */
#if RV32_HAS(EXT_V)
            case 0x13:
                ir->vd = decode_rd(insn);
                ir->vs2 = decode_rs2(insn);
                ir->rs1 = decode_rs1(insn);
                ir->vm = (insn >> 25) & 0x1;
                ir->opcode = rv_insn_vmsbc_vx;
                return true;
#endif /* RV32_HAS(EXT_V) */
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    return false;
}
#else /* !RV32_HAS(EXT_V) */
#define op_op_v op_unimp
#endif /* RV32_HAS(EXT_V) */

static inline bool op_branch(rv_insn_t *ir, const uint32_t insn)
{
    decode_btype(ir, insn);

    switch ((insn >> 12) & 0x7) {
    case 0:
        ir->opcode = rv_insn_beq;
        return true;
    case 1:
        ir->opcode = rv_insn_bne;
        return true;
    case 4:
        ir->opcode = rv_insn_blt;
        return true;
    case 5:
        ir->opcode = rv_insn_bge;
        return true;
    case 6:
        ir->opcode = rv_insn_bltu;
        return true;
    case 7:
        ir->opcode = rv_insn_bgeu;
        return true;
    default:
        break;
    }
    return false;
}

static inline bool op_jalr(rv_insn_t *ir, const uint32_t insn)
{
    decode_itype(ir, insn);

    switch ((insn >> 12) & 0x7) {
    case 0:
        ir->opcode = rv_insn_jalr;
        return true;
    default:
        break;
    }
    return false;
}

static inline bool op_jal(rv_insn_t *ir, const uint32_t insn)
{
    decode_jtype(ir, insn);
    ir->opcode = rv_insn_jal;
    return true;
}

static inline bool op_system(rv_insn_t *ir, const uint32_t insn)
{
    decode_itype(ir, insn);

    switch ((insn >> 7) & 0x1f) {
    case 0:
        switch ((insn >> 12) & 0x7) {
        case 0:
            switch ((insn >> 25) & 0x7f) {
            case 0x9:
                ir->opcode = rv_insn_sfencevma;
                return true;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 12) & 0x7) {
    case 0:
        switch ((insn >> 20) & 0xfff) {
        case 0:
            switch ((insn >> 15) & 0x1f) {
            case 0:
                switch ((insn >> 7) & 0x1f) {
                case 0:
                    ir->opcode = rv_insn_ecall;
                    return true;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 1:
            switch ((insn >> 15) & 0x1f) {
            case 0:
                switch ((insn >> 7) & 0x1f) {
                case 0:
                    ir->opcode = rv_insn_ebreak;
                    return true;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x102:
            switch ((insn >> 15) & 0x1f) {
            case 0:
                switch ((insn >> 7) & 0x1f) {
#if RV32_HAS(SYSTEM)
                case 0:
                    ir->opcode = rv_insn_sret;
                    return true;
#endif /* RV32_HAS(SYSTEM) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x105:
            switch ((insn >> 15) & 0x1f) {
            case 0:
                switch ((insn >> 7) & 0x1f) {
                case 0:
                    ir->opcode = rv_insn_wfi;
                    return true;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        case 0x302:
            switch ((insn >> 15) & 0x1f) {
            case 0:
                switch ((insn >> 7) & 0x1f) {
                case 0:
                    ir->opcode = rv_insn_mret;
                    return true;
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
#if RV32_HAS(Zicsr)
    case 1:
        ir->opcode = rv_insn_csrrw;
        return csr_is_writable(ir->imm) || (ir->rs1 == rv_reg_zero);
#endif /* RV32_HAS(Zicsr) */
#if RV32_HAS(Zicsr)
    case 2:
        ir->opcode = rv_insn_csrrs;
        return csr_is_writable(ir->imm) || (ir->rs1 == rv_reg_zero);
#endif /* RV32_HAS(Zicsr) */
#if RV32_HAS(Zicsr)
    case 3:
        ir->opcode = rv_insn_csrrc;
        return csr_is_writable(ir->imm) || (ir->rs1 == rv_reg_zero);
#endif /* RV32_HAS(Zicsr) */
#if RV32_HAS(Zicsr)
    case 5:
        ir->opcode = rv_insn_csrrwi;
        return csr_is_writable(ir->imm) || (ir->rs1 == rv_reg_zero);
#endif /* RV32_HAS(Zicsr) */
#if RV32_HAS(Zicsr)
    case 6:
        ir->opcode = rv_insn_csrrsi;
        return csr_is_writable(ir->imm) || (ir->rs1 == rv_reg_zero);
#endif /* RV32_HAS(Zicsr) */
#if RV32_HAS(Zicsr)
    case 7:
        ir->opcode = rv_insn_csrrci;
        return csr_is_writable(ir->imm) || (ir->rs1 == rv_reg_zero);
#endif /* RV32_HAS(Zicsr) */
    default:
        break;
    }
    return false;
}

#if RV32_HAS(EXT_C)

/* RVC field decoders */
static inline uint16_t c_decode_rs1(const uint16_t insn)
{
    return (insn >> 7) & 0x1f;
}
static inline uint16_t c_decode_rs2(const uint16_t insn)
{
    return (insn >> 2) & 0x1f;
}
static inline uint16_t c_decode_rd(const uint16_t insn)
{
    return (insn >> 7) & 0x1f;
}
static inline uint16_t c_decode_rs1c(const uint16_t insn)
{
    return (insn >> 7) & 0x7;
}
static inline uint16_t c_decode_rs2c(const uint16_t insn)
{
    return (insn >> 2) & 0x7;
}
static inline uint16_t c_decode_rdc(const uint16_t insn)
{
    return (insn >> 2) & 0x7;
}

static inline bool op_rvc_q0(rv_insn_t *ir, const uint32_t insn)
{
    switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
    case 0:
        ir->rd = c_decode_rdc(insn) | 0x08;
        ir->imm = ((insn & 0x1800) >> 7) | ((insn & 0x0780) >> 1) |
                  ((insn & 0x0040) >> 4) | ((insn & 0x0020) >> 2);
        ir->rs1 = rv_reg_sp;
        if (unlikely(!ir->imm))
            return false;
        ir->opcode = rv_insn_caddi4spn;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_C)
    case 2:
        ir->rd = c_decode_rdc(insn) | 0x08;
        ir->rs1 = c_decode_rs1c(insn) | 0x08;
        ir->imm = ((insn & 0x1C00) >> 7) | ((insn & 0x0040) >> 4) |
                  ((insn & 0x0020) << 1);
        ir->opcode = rv_insn_clw;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_F)
    case 3:
        ir->rd = c_decode_rdc(insn) | 0x08;
        ir->rs1 = c_decode_rs1c(insn) | 0x08;
        ir->imm = ((insn & 0x1C00) >> 7) | ((insn & 0x0040) >> 4) |
                  ((insn & 0x0020) << 1);
        ir->opcode = rv_insn_cflw;
        return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_C)
    case 6:
        ir->rs1 = c_decode_rs1c(insn) | 0x08;
        ir->rs2 = c_decode_rs2c(insn) | 0x08;
        ir->imm = ((insn & 0x1C00) >> 7) | ((insn & 0x0040) >> 4) |
                  ((insn & 0x0020) << 1);
        ir->opcode = rv_insn_csw;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_F)
    case 7:
        ir->rs1 = c_decode_rs1c(insn) | 0x08;
        ir->rs2 = c_decode_rs2c(insn) | 0x08;
        ir->imm = ((insn & 0x1C00) >> 7) | ((insn & 0x0040) >> 4) |
                  ((insn & 0x0020) << 1);
        ir->opcode = rv_insn_cfsw;
        return true;
#endif /* RV32_HAS(EXT_F) */
    default:
        break;
    }
    return false;
}

static inline bool op_rvc_q1(rv_insn_t *ir, const uint32_t insn)
{
    switch ((insn >> 2) & 0x1f) {
    case 0:
        switch ((insn >> 7) & 0x1f) {
        case 0:
            switch ((insn >> 12) & 0x1) {
            case 0:
                switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
                case 0:
                    ir->rd = rv_reg_zero;
                    ir->rs1 = rv_reg_zero;
                    ir->opcode = rv_insn_cnop;
                    return true;
#endif /* RV32_HAS(EXT_C) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 5) & 0x3) {
    case 0:
        switch ((insn >> 10) & 0x3) {
        case 3:
            switch ((insn >> 12) & 0x1) {
            case 0:
                switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
                case 4:
                    ir->rs1 = c_decode_rs1c(insn) | 0x08;
                    ir->rs2 = c_decode_rs2c(insn) | 0x08;
                    ir->rd = ir->rs1;
                    ir->opcode = rv_insn_csub;
                    return true;
#endif /* RV32_HAS(EXT_C) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 1:
        switch ((insn >> 10) & 0x3) {
        case 3:
            switch ((insn >> 12) & 0x1) {
            case 0:
                switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
                case 4:
                    ir->rs1 = c_decode_rs1c(insn) | 0x08;
                    ir->rs2 = c_decode_rs2c(insn) | 0x08;
                    ir->rd = ir->rs1;
                    ir->opcode = rv_insn_cxor;
                    return true;
#endif /* RV32_HAS(EXT_C) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 2:
        switch ((insn >> 10) & 0x3) {
        case 3:
            switch ((insn >> 12) & 0x1) {
            case 0:
                switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
                case 4:
                    ir->rs1 = c_decode_rs1c(insn) | 0x08;
                    ir->rs2 = c_decode_rs2c(insn) | 0x08;
                    ir->rd = ir->rs1;
                    ir->opcode = rv_insn_cor;
                    return true;
#endif /* RV32_HAS(EXT_C) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 3:
        switch ((insn >> 10) & 0x3) {
        case 3:
            switch ((insn >> 12) & 0x1) {
            case 0:
                switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
                case 4:
                    ir->rs1 = c_decode_rs1c(insn) | 0x08;
                    ir->rs2 = c_decode_rs2c(insn) | 0x08;
                    ir->rd = ir->rs1;
                    ir->opcode = rv_insn_cand;
                    return true;
#endif /* RV32_HAS(EXT_C) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 7) & 0x1f) {
    case 2:
        switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
        case 3:
            ir->imm = ((insn & 0x0040) >> 2) | ((insn & 0x0020) << 1) |
                      ((insn & 0x0018) << 4) | ((insn & 0x0004) << 3) |
                      ((insn & 0x1000) >> 3);
            ir->imm |= -(ir->imm & 0x200);
            ir->rd = rv_reg_sp;
            ir->rs1 = rv_reg_sp;
            if (unlikely(!ir->imm))
                return false;
            ir->opcode = rv_insn_caddi16sp;
            return true;
#endif /* RV32_HAS(EXT_C) */
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 10) & 0x3) {
    case 0:
        switch ((insn >> 12) & 0x1) {
        case 0:
            switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
            case 4:
                ir->rs1 = c_decode_rs1c(insn) | 0x08;
                ir->shamt = ((insn >> 7) & 0x20) | ((insn >> 2) & 0x1f);
                ir->imm = ir->shamt;
                ir->rd = ir->rs1;
                ir->opcode = rv_insn_csrli;
                return true;
#endif /* RV32_HAS(EXT_C) */
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 1:
        switch ((insn >> 12) & 0x1) {
        case 0:
            switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
            case 4:
                ir->rs1 = c_decode_rs1c(insn) | 0x08;
                ir->shamt = ((insn >> 7) & 0x20) | ((insn >> 2) & 0x1f);
                ir->imm = ir->shamt;
                ir->rd = ir->rs1;
                ir->opcode = rv_insn_csrai;
                return true;
#endif /* RV32_HAS(EXT_C) */
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    case 2:
        switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
        case 4:
            ir->rs1 = c_decode_rs1c(insn) | 0x08;
            ir->imm = ((insn >> 2) & 0x1f) | ((insn & 0x1000) >> 7);
            ir->imm |= -(ir->imm & 0x20);
            ir->rd = ir->rs1;
            ir->opcode = rv_insn_candi;
            return true;
#endif /* RV32_HAS(EXT_C) */
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
    case 0:
        ir->rd = c_decode_rd(insn);
        ir->rs1 = ir->rd;
        ir->imm = ((insn >> 2) & 0x1f) | ((insn & 0x1000) >> 7);
        ir->imm |= -(ir->imm & 0x20);
        if (unlikely(ir->rd == rv_reg_zero)) {
            ir->opcode = rv_insn_cnop;
            return true;
        }
        ir->opcode = rv_insn_caddi;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_C)
    case 1:
        ir->imm = ((insn & 0x0800) >> 7) | ((insn & 0x0600) >> 1) |
                  ((insn & 0x0100) << 2) | ((insn & 0x0080) >> 1) |
                  ((insn & 0x0040) << 1) | ((insn & 0x0038) >> 2) |
                  ((insn & 0x0004) << 3) | ((insn & 0x1000) >> 1);
        ir->imm |= -(ir->imm & 0x800);
        ir->rd = rv_reg_ra;
        ir->opcode = rv_insn_cjal;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_C)
    case 2:
        ir->rd = c_decode_rd(insn);
        ir->imm = ((insn >> 2) & 0x1f) | ((insn & 0x1000) >> 7);
        ir->imm |= -(ir->imm & 0x20);
        ir->rs1 = rv_reg_zero;
        if (unlikely(ir->rd == rv_reg_zero)) {
            ir->opcode = rv_insn_cnop;
            return true;
        }
        ir->opcode = rv_insn_cli;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_C)
    case 3:
        ir->rd = c_decode_rd(insn);
        ir->imm = ((insn >> 2) & 0x1f) | ((insn & 0x1000) >> 7);
        ir->imm |= -(ir->imm & 0x20);
        ir->imm <<= 12;
        if (unlikely(ir->rd == rv_reg_zero)) {
            ir->opcode = rv_insn_cnop;
            return true;
        }
        if (unlikely(!ir->imm))
            return false;
        ir->opcode = rv_insn_clui;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_C)
    case 5:
        ir->imm = ((insn & 0x0800) >> 7) | ((insn & 0x0600) >> 1) |
                  ((insn & 0x0100) << 2) | ((insn & 0x0080) >> 1) |
                  ((insn & 0x0040) << 1) | ((insn & 0x0038) >> 2) |
                  ((insn & 0x0004) << 3) | ((insn & 0x1000) >> 1);
        ir->imm |= -(ir->imm & 0x800);
        ir->rd = rv_reg_zero;
        ir->opcode = rv_insn_cj;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_C)
    case 6:
        ir->rs1 = c_decode_rs1c(insn) | 0x08;
        ir->imm = ((insn & 0x0C00) >> 7) | ((insn & 0x0060) << 1) |
                  ((insn & 0x0018) >> 2) | ((insn & 0x0004) << 3) |
                  ((insn & 0x1000) >> 4);
        ir->imm |= -(ir->imm & 0x100);
        ir->rs2 = rv_reg_zero;
        ir->opcode = rv_insn_cbeqz;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_C)
    case 7:
        ir->rs1 = c_decode_rs1c(insn) | 0x08;
        ir->imm = ((insn & 0x0C00) >> 7) | ((insn & 0x0060) << 1) |
                  ((insn & 0x0018) >> 2) | ((insn & 0x0004) << 3) |
                  ((insn & 0x1000) >> 4);
        ir->imm |= -(ir->imm & 0x100);
        ir->rs2 = rv_reg_zero;
        ir->opcode = rv_insn_cbnez;
        return true;
#endif /* RV32_HAS(EXT_C) */
    default:
        break;
    }
    return false;
}

static inline bool op_rvc_q2(rv_insn_t *ir, const uint32_t insn)
{
    switch ((insn >> 2) & 0x1f) {
    case 0:
        switch ((insn >> 7) & 0x1f) {
        case 0:
            switch ((insn >> 12) & 0x1) {
            case 1:
                switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
                case 4:
                    ir->opcode = rv_insn_cebreak;
                    return true;
#endif /* RV32_HAS(EXT_C) */
                default:
                    break;
                }
                break;
            default:
                break;
            }
            break;
        default:
            break;
        }
        switch ((insn >> 12) & 0x1) {
        case 0:
            switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
            case 4:
                ir->rs1 = c_decode_rs1(insn);
                ir->rd = rv_reg_zero;
                if (unlikely(ir->rs1 == rv_reg_zero))
                    return false;
                ir->opcode = rv_insn_cjr;
                return true;
#endif /* RV32_HAS(EXT_C) */
            default:
                break;
            }
            break;
        case 1:
            switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
            case 4:
                ir->rs1 = c_decode_rs1(insn);
                ir->rd = rv_reg_ra;
                ir->opcode = rv_insn_cjalr;
                return true;
#endif /* RV32_HAS(EXT_C) */
            default:
                break;
            }
            break;
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 12) & 0x1) {
    case 0:
        switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
        case 0:
            ir->rd = c_decode_rd(insn);
            ir->rs1 = ir->rd;
            ir->shamt = ((insn >> 7) & 0x20) | ((insn >> 2) & 0x1f);
            ir->imm = ir->shamt;
            if (unlikely(ir->rd == rv_reg_zero)) {
                ir->opcode = rv_insn_cnop;
                return true;
            }
            ir->opcode = rv_insn_cslli;
            return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_C)
        case 4:
            ir->rd = c_decode_rd(insn);
            ir->rs2 = c_decode_rs2(insn);
            ir->rs1 = rv_reg_zero;
            if (unlikely(ir->rd == rv_reg_zero)) {
                ir->opcode = rv_insn_cnop;
                return true;
            }
            ir->opcode = rv_insn_cmv;
            return true;
#endif /* RV32_HAS(EXT_C) */
        default:
            break;
        }
        break;
    case 1:
        switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
        case 4:
            ir->rd = c_decode_rd(insn);
            ir->rs1 = ir->rd;
            ir->rs2 = c_decode_rs2(insn);
            if (unlikely(ir->rd == rv_reg_zero)) {
                ir->opcode = rv_insn_cnop;
                return true;
            }
            ir->opcode = rv_insn_cadd;
            return true;
#endif /* RV32_HAS(EXT_C) */
        default:
            break;
        }
        break;
    default:
        break;
    }
    switch ((insn >> 13) & 0x7) {
#if RV32_HAS(EXT_C)
    case 2:
        ir->rd = c_decode_rd(insn);
        ir->imm = ((insn & 0x1000) >> 7) | ((insn & 0x0070) >> 2) |
                  ((insn & 0x000C) << 4);
        ir->rs1 = rv_reg_sp;
        if (unlikely(ir->rd == rv_reg_zero))
            return false;
        ir->opcode = rv_insn_clwsp;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_F)
    case 3:
        ir->rd = c_decode_rd(insn);
        ir->imm = ((insn & 0x1000) >> 7) | ((insn & 0x0070) >> 2) |
                  ((insn & 0x000C) << 4);
        ir->rs1 = rv_reg_sp;
        ir->opcode = rv_insn_cflwsp;
        return true;
#endif /* RV32_HAS(EXT_F) */
#if RV32_HAS(EXT_C)
    case 6:
        ir->rs2 = c_decode_rs2(insn);
        ir->imm = ((insn & 0x1E00) >> 7) | ((insn & 0x0180) >> 1);
        ir->rs1 = rv_reg_sp;
        ir->opcode = rv_insn_cswsp;
        return true;
#endif /* RV32_HAS(EXT_C) */
#if RV32_HAS(EXT_F)
    case 7:
        ir->rs2 = c_decode_rs2(insn);
        ir->imm = ((insn & 0x1E00) >> 7) | ((insn & 0x0180) >> 1);
        ir->rs1 = rv_reg_sp;
        ir->opcode = rv_insn_cfswsp;
        return true;
#endif /* RV32_HAS(EXT_F) */
    default:
        break;
    }
    return false;
}

#endif /* RV32_HAS(EXT_C) */

static inline bool op_unimp(rv_insn_t *ir UNUSED, uint32_t insn UNUSED)
{
    return false;
}

typedef bool (*decode_t)(rv_insn_t *ir, uint32_t insn);

static const decode_t rv_jump_table[] = {
    op_load,     /* 0 */
    op_load_fp,  /* 1 */
    op_unimp,    /* 2 */
    op_misc_mem, /* 3 */
    op_op_imm,   /* 4 */
    op_auipc,    /* 5 */
    op_unimp,    /* 6 */
    op_unimp,    /* 7 */
    op_store,    /* 8 */
    op_store_fp, /* 9 */
    op_unimp,    /* 10 */
    op_amo,      /* 11 */
    op_op,       /* 12 */
    op_lui,      /* 13 */
    op_unimp,    /* 14 */
    op_unimp,    /* 15 */
    op_madd,     /* 16 */
    op_msub,     /* 17 */
    op_nmsub,    /* 18 */
    op_nmadd,    /* 19 */
    op_op_fp,    /* 20 */
    op_op_v,     /* 21 */
    op_unimp,    /* 22 */
    op_unimp,    /* 23 */
    op_branch,   /* 24 */
    op_jalr,     /* 25 */
    op_unimp,    /* 26 */
    op_jal,      /* 27 */
    op_system,   /* 28 */
    op_unimp,    /* 29 */
    op_unimp,    /* 30 */
    op_unimp,    /* 31 */
};

#if RV32_HAS(EXT_C)
static const decode_t rvc_jump_table[] = {
    op_rvc_q0, op_rvc_q1, op_rvc_q2, op_unimp, /* funct3=0 */
    op_rvc_q0, op_rvc_q1, op_rvc_q2, op_unimp, /* funct3=1 */
    op_rvc_q0, op_rvc_q1, op_rvc_q2, op_unimp, /* funct3=2 */
    op_rvc_q0, op_rvc_q1, op_rvc_q2, op_unimp, /* funct3=3 */
    op_rvc_q0, op_rvc_q1, op_rvc_q2, op_unimp, /* funct3=4 */
    op_rvc_q0, op_rvc_q1, op_rvc_q2, op_unimp, /* funct3=5 */
    op_rvc_q0, op_rvc_q1, op_rvc_q2, op_unimp, /* funct3=6 */
    op_rvc_q0, op_rvc_q1, op_rvc_q2, op_unimp, /* funct3=7 */
};
#endif /* RV32_HAS(EXT_C) */

bool rv_decode(rv_insn_t *ir, uint32_t insn)
{
    bool ret;
    assert(ir);

#if RV32_HAS(EXT_C)
    if (is_compressed(insn)) {
        insn &= 0x0000FFFF;
        const uint16_t c_index = ((insn & 0xE000) >> 11) | (insn & 0x3);
        ret = rvc_jump_table[c_index](ir, insn);
        goto end;
    }
#endif

    const uint32_t index = (insn >> 2) & 0x1f;
    ret = rv_jump_table[index](ir, insn);

end:

#if RV32_HAS(RV32E)
    if (unlikely(ir->rd > 15 || ir->rs1 > 15 || ir->rs2 > 15))
        ret = false;
#endif

    return ret;
}
