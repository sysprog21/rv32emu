 ...
 * @freq: receives the installed entry's frequency, which a revived ghost
 *        inherits; zero when nothing was inserted
 * @cache: a pointer points to target cache
 * @key: the key of the inserted entry
 * @value: the value of the inserted entry
 * @return: the replaced entry or NULL
 */
void *cache_put(struct cache *cache, uint32_t key, void *value, uint32_t *freq);
 ...
 * one hash lookup. The two fields are returned in registers on supported
