...
typedef enum {
    VSND_CANCEL_DRAIN_TX,
    VSND_CANCEL_TX_QUIESCED,
} vsnd_cancel_mode_t;

typedef union {
    virtio_snd_hdr_t hdr;
    virtio_snd_query_info_t query;
    virtio_snd_pcm_set_params_t set_params;
    virtio_snd_pcm_hdr_t pcm;
} vsnd_ctrl_request_t;

typedef union {
    virtio_snd_jack_info_t jack[VSND_DEV_CNT_MAX];
    virtio_snd_pcm_info_t pcm[VSND_DEV_CNT_MAX];
    virtio_snd_chmap_info_t chmap[VSND_DEV_CNT_MAX];
} vsnd_ctrl_info_t;
...
{
    /*
     * list_add() inserts after the list head. The oldest PCM buffer therefore
     * remains at the tail and is consumed through list_last_entry().
     */
    list_add(&node->q, &props->buf_queue);
}

static vsnd_buf_queue_node_t *vsnd_queue_front(virtio_snd_prop_t *props)
{
    if (list_empty(&props->buf_queue))
        return NULL;

    return list_last_entry(&props->buf_queue, vsnd_buf_queue_node_t, q);
}

static void vsnd_queue_remove(vsnd_buf_queue_node_t *node)
{
    list_del(&node->q);
}
...
{
    /*
     * Use the same FIFO convention as the PCM queue: newest entries are added
     * after the head and the oldest entry is removed from the tail.
     */
    list_add(&node->q, &props->completed_queue);
}

static vsnd_buf_queue_node_t *vsnd_completed_pop_locked(
    virtio_snd_prop_t *props)
{
    if (list_empty(&props->completed_queue))
        return NULL;

    vsnd_buf_queue_node_t *node =
        list_last_entry(&props->completed_queue, vsnd_buf_queue_node_t, q);
    list_del(&node->q);
    return node;
}
...
{
    while (!list_empty(&props->buf_queue)) {
        vsnd_buf_queue_node_t *node = vsnd_queue_front(props);
        vsnd_queue_remove(node);
...
static void vsnd_clear_all_nodes_locked(virtio_snd_prop_t *props)
{
    ...
    while (!list_empty(&props->completed_queue)) {
        vsnd_buf_queue_node_t *node =
            list_last_entry(&props->completed_queue, vsnd_buf_queue_node_t, q);
        list_del(&node->q);
        vsnd_free_node(node);
...
static void vsnd_reset_stream(virtio_snd_prop_t *props)
{
    ...
    /*
     * Device reset/delete already prevents new TX processing before this
     * function drains host-owned requests.
     */
    vsnd_cancel_pending_and_wait(props, VSND_CANCEL_TX_QUIESCED);
...
static void virtio_snd_update_status(virtio_snd_state_t *vsnd, uint32_t status)
{
    ...
    if (priv) {
        ...
        /*
         * Stop the TX worker from consuming any more descriptors from the
         * old avail ring while the streams are being drained.
         */
        pthread_mutex_lock(&p->tx_process_mutex);

        /*
         * Discard notifications that have not yet been consumed by the TX
         * worker. A worker that already passed this point may still block on
         * tx_process_mutex, but after reset it will observe status == 0 and
         * leave the TX queue untouched.
         */
        pthread_mutex_lock(&p->tx_mutex);
        p->tx_ev_notify = false;
        pthread_mutex_unlock(&p->tx_mutex);

        /*
         * No new TX request can enter the host side now. Drain requests
         * already owned by the device and wait for their completions.
         */
        for (uint32_t i = 0; i < VSND_DEV_CNT_MAX; i++)
            vsnd_reset_stream(&p->props[i]);

        /*
         * All in-flight requests have completed. Prevent any used-ring
         * publication while the guest-visible device state is cleared.
         *
         * This must be acquired after draining: the completion thread needs
         * tx_used_mutex in order to make inflight_count reach zero.
         */
...
static bool vsnd_guest_read(virtio_snd_state_t *vsnd,
                            uint64_t addr,
                            void *dst,
                            uint32_t len)
{
    void *src = vsnd_guest_ptr(vsnd, addr, len);
    if (!src)
        return false;
...
static bool vsnd_guest_write(virtio_snd_state_t *vsnd,
                             uint64_t addr,
                             const void *src,
                             uint32_t len)
{
    void *dst = vsnd_guest_ptr(vsnd, addr, len);
    if (!dst)
        return false;
                                           ...
                                           uint32_t latency_bytes)
{
    ...
    if (!vsnd_guest_write(vsnd, status_addr, &status, sizeof(status))) {
        pthread_mutex_unlock(&p->tx_used_mutex);
        return false;
    }
    ...
    used_idx++;
    uint32_t used_word =
        (ram[queue->queue_used] & MASK(16)) | ((uint32_t) used_idx << 16);
    __atomic_store_n(&ram[queue->queue_used], used_word, __ATOMIC_RELEASE);
...
static void *virtio_snd_completion_thread(void *arg)
{
    ...
    for (;;) {
        pthread_mutex_lock(&props->lock.lock);
        while (list_empty(&props->completed_queue) &&
               !props->completion_thread_stop)
            pthread_cond_wait(&props->lock.completed, &props->lock.lock);

        if (props->completion_thread_stop &&
            list_empty(&props->completed_queue)) {
            pthread_mutex_unlock(&props->lock.lock);
            break;
        }
                                         ...
                                         vsnd_cancel_mode_t mode)
{
    ...
    /*
     * Stream control operations keep the same virtqueue alive, so drain
     * descriptors already published by the guest. Device reset instead
     * quiesces TX processing before entering this function and discards
     * descriptors that were never taken from the old avail ring.
     */
    if (mode == VSND_CANCEL_DRAIN_TX && props->vsnd)
                                    ...
                                    uint32_t *chain_cnt)
{
    ...
    for (;;) {
        ...
        memcpy(&desc, &vsnd->ram[desc_word], sizeof(desc));
    ...
    uint32_t *payload_len)
{
    ...
    if (!request->buffer_bytes || !request->period_bytes ||
        request->period_bytes > request->buffer_bytes)
        return VIRTIO_SND_S_BAD_MSG;
                                       ...
                                       uint32_t *payload_len)
{
    ...
    vsnd_cancel_pending_and_wait(props, VSND_CANCEL_DRAIN_TX);
                                       ...
                                       uint32_t bytes)
{
    ...
    uint32_t copied = 0;
    while (!list_empty(&props->buf_queue) && copied < bytes) {
        vsnd_buf_queue_node_t *node = vsnd_queue_front(props);
                                      ...
                                      uint32_t *plen)
{
    ...
    *plen = 0;
    ...
    if (!vsnd_guest_read(vsnd, xfer_desc->addr, &xfer, sizeof(xfer)))
        return -1;
    if (!vsnd_guest_range_ok(status_desc->addr,
    ...
    bool queue_has_room =
        total64 <= buffer_bytes && props->lock.pending_pcm_bytes <=
                                       (size_t) buffer_bytes - (size_t) total64;
    ...
    if (total64 > period_bytes) {
        virtio_snd_complete_tx_request(vsnd, desc_idx, status_desc->addr,
                                       VIRTIO_SND_S_BAD_MSG, 0);
        return 0;
    }
    ...
    queue_has_room = node->len <= props->pp.buffer_bytes &&
                     props->lock.pending_pcm_bytes <=
                         (size_t) props->pp.buffer_bytes - node->len;

    if (accepting && queue_has_room) {
        vsnd_queue_push(props, node);
        props->lock.pending_pcm_bytes += node->len;
    } else {
        if (accepting)
            node->completion_status = VIRTIO_SND_S_IO_ERR;
        vsnd_completed_push_locked(props, node);
        pthread_cond_signal(&props->lock.completed);
    }
    pthread_mutex_unlock(&props->lock.lock);
                                        ...
                                        uint32_t *plen)
{
    ...
    vsnd_ctrl_request_t request;
    memset(&request, 0, sizeof(request));

    if (!vsnd_guest_read(vsnd, request_desc->addr, &request.hdr,
                         sizeof(request.hdr)))
        return -1;

    uint32_t type = request.hdr.code;
    uint32_t request_size = sizeof(virtio_snd_hdr_t);

    switch (type) {
    case VIRTIO_SND_R_JACK_INFO:
    case VIRTIO_SND_R_PCM_INFO:
    case VIRTIO_SND_R_CHMAP_INFO:
        request_size = sizeof(virtio_snd_query_info_t);
        break;
    case VIRTIO_SND_R_PCM_SET_PARAMS:
        request_size = sizeof(virtio_snd_pcm_set_params_t);
        break;
    case VIRTIO_SND_R_PCM_PREPARE:
    case VIRTIO_SND_R_PCM_RELEASE:
    case VIRTIO_SND_R_PCM_START:
    case VIRTIO_SND_R_PCM_STOP:
        request_size = sizeof(virtio_snd_pcm_hdr_t);
        break;
    default:
        break;
    }

    if (request_desc->len < request_size)
        return -1;

    if (!vsnd_guest_read(vsnd, request_desc->addr, &request, request_size))
        return -1;

    /*
     * INFO responses are also built in aligned host storage, then copied
     * back to guest memory after the control request has been processed.
     */
    vsnd_ctrl_info_t info_buf;
    memset(&info_buf, 0, sizeof(info_buf));

    struct virtq_desc *info_desc = NULL;
    void *info = NULL;
    uint32_t info_len = 0;
    ...
    if (cnt >= 3) {
        ...
        if (!(info_desc->flags & VIRTIO_DESC_F_WRITE))
            return -1;
        if (!vsnd_guest_range_ok(info_desc->addr, info_desc->len))
            return -1;
        info = &info_buf;
    ...
    uint32_t status = virtio_snd_ctrl_process(vsnd, &request, info, type,
                                              info_len, &payload_len);

    /*
     * Copy INFO payload produced by the handler back into guest memory.
     */
    if (payload_len) {
        if (!info_desc || payload_len > info_desc->len ||
            payload_len > sizeof(info_buf))
            return -1;

        if (!vsnd_guest_write(vsnd, info_desc->addr, &info_buf, payload_len))
            return -1;
    }

    /*
     * Build the control response locally as well, so the guest-provided
     * response address does not need to satisfy host struct alignment.
     */
    virtio_snd_hdr_t response = {
        .code = status,
    };

    if (!vsnd_guest_write(vsnd, response_desc->addr, &response,
                          sizeof(response)))
        return -1;
...
static void virtio_queue_notify_handler(virtio_snd_state_t *vsnd, int index)
{
    ...
    if (async_tx)
        return;
    uint32_t used_word =
        (ram[queue->queue_used] & MASK(16)) | ((uint32_t) new_used << 16);
    __atomic_store_n(&ram[queue->queue_used], used_word, __ATOMIC_RELEASE);
    if (!(ram[queue->queue_avail] & 1))
        __atomic_fetch_or(&vsnd->interrupt_status, VIRTIO_INT_USED_RING,
                          __ATOMIC_RELEASE);
...
static void *virtio_snd_tx_thread(void *arg)
{
    ...
    for (;;) {
        ...
        while (!p->tx_ev_notify && !p->tx_thread_stop)
...
static bool vsnd_init_prop(virtio_snd_prop_t *props)
{
    ...
    INIT_LIST_HEAD(&props->buf_queue);
    INIT_LIST_HEAD(&props->completed_queue);
...
void virtio_snd_reset(virtio_snd_state_t *vsnd)
{
    if (!vsnd)
        return;

    virtio_snd_update_status(vsnd, 0);
}
