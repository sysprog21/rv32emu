 ...
 *   })
 *
 * The block defined as 'X64' is mapped to the generic C code used in the
 * interpreter. The following instructions will be generated by JIT compiler:
 * - Load X->rs1 (target field) from the rv data structure to RAX (destination
 *   register).
 * - Do ALU operation on 0 (source register) and imm and store the result into
 *   RAX (destination register).
 * - Store RAX (source register) value to the X->rd (target field) of the rv
 * data structure.
 *
