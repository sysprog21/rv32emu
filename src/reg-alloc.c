...
static inline void reset_liveness()
{
...
static inline void init_candidate_queue()
{
    for (int i = 0; i < N_RV_REGS; i++) {
        candidate_queue[i] = i;
    }
...
static inline void calc_liveness(block_t *block)
{
    ...
    init_candidate_queue();
    qsort(candidate_queue, N_RV_REGS, sizeof(uint8_t), compare_liveness);
}

static inline void refresh_regs(int idx)
{
...
static inline int pick_reg(int reserved)
{
