...
# Register before executing: with deferred annotations, dataclasses
# resolves a field's type by looking the defining module up in
# sys.modules, which fails for a module loaded only through a spec.
...
_DEFAULT_KEY = _mod._DEFAULT_KEY
    ...
    for hi, lo in sorted(bit_range_keys):
        ...
        if extracted in subtree:
            ...
            if result is not None:
                return result
        elif _DEFAULT_KEY in subtree:
            # No case label matched, so the generated switch lands on its
            # "default:" label, which holds the less-specific instruction.
            # This is deliberately an "elif": once a case label matches,
            # the generated C breaks out of the switch and never reaches
            # "default:", so walking it here would accept words the
            # decoder rejects.
            result = walk_tree(subtree[_DEFAULT_KEY], word)
            if result is not None:
                return result
